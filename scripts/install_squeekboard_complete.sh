...
# Disable onboard if it's installed
if command_exists onboard; then
    echo "Onboard is installed. Disabling it to avoid conflicts..."
...
if [ -d "../central_system" ] || [ -d "central_system" ]; then
...
EOF
        ...
        if grep -q "CONSULTEASE_KEYBOARD=" "$ENV_PATH"; then
            sed -i 's/CONSULTEASE_KEYBOARD=.*/CONSULTEASE_KEYBOARD=squeekboard/' "$ENV_PATH"
        else
            echo "CONSULTEASE_KEYBOARD=squeekboard" >> "$ENV_PATH"
        fi

        if grep -q "SQUEEKBOARD_FORCE=" "$ENV_PATH"; then
            sed -i 's/SQUEEKBOARD_FORCE=.*/SQUEEKBOARD_FORCE=1/' "$ENV_PATH"
        else
            echo "SQUEEKBOARD_FORCE=1" >> "$ENV_PATH"
        fi

        if grep -q "ONBOARD_DISABLE=" "$ENV_PATH"; then
            sed -i 's/ONBOARD_DISABLE=.*/ONBOARD_DISABLE=1/' "$ENV_PATH"
        else
            echo "ONBOARD_DISABLE=1" >> "$ENV_PATH"
        fi
    fi
